...
class TopicCrudController extends Controller
{
    ...
    public function index(TopicFilters $filters)
    {
        return Inertia::render('Admin/Topics', [
            ...
            'topics' => function () use($filters) {
                return fractal(Topic::with('skill')
                    ->filter($filters)
    ...
    // In app/Http/Controllers/Admin/TopicCrudController.php
public function simpleIndex()
{
    $topics = Topic::with('skill')
        ->paginate(10)
...
{
    $topics = \App\Models\Topic::with('skill')->paginate(10);
    return view('admin.topics.simple-list', compact('topics')); // Remove 'vendor' from path
}
