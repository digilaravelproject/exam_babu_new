<template>
    <nav :class="{'flex-1 bg-primary': !isMobile}" class="relative px-4 space-y-3">
        <div class="bg-gray-100 bg-opacity-10 rounded-md py-3 px-4" style="visibility: hidden;">
...
<inertia-link :href="route('add_exams')"
                      :class="[route().current() === 'add_exams' ? 'bg-secondary text-white' : 'text-white hover:bg-secondary hover:text-white']"
                      class="group rounded-md py-3 px-4 flex items-center text-base font-semibold">
            <svg :class="[route().current() === 'add_exams' ? 'text-white' : 'text-secondary']" class="group-hover:text-white ltr:mr-3 rtl:ml-3 flex-shrink-0 h-6 w-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
            {{ __('Add Exams') }}
        </inertia-link>
