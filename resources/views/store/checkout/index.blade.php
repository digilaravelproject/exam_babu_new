                                    ...
                                    @foreach($paymentProcessors as $processor)
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center cursor-pointer">
                                                <input x-model="payment" id="processor_{{ $processor['code'] }}" name="payment_method" type="radio" value="{{ $processor['code'] }}" {{ $processor['default'] ? 'checked' : '' }} class="cursor-pointer focus:ring-current h-4 w-4 text-primary border-gray-300">
                                        ...
                                        <div class="mt-1">
                                            <select id="country" name="country" value="{{ old('country') ?? ($billing_information['country'] ?? null) }}" autocomplete="country" class="block w-full rounded-sm shadow-sm focus:ring-current focus:border-primary {{ $errors->has('country') ? ' border-red-300' : 'border-gray-300' }}">
                                                @foreach($countries as $country)
                                                    <option value="{{ $country['code'] }}" {{ $country['code'] == 'IN' ? 'selected' : '' }}>{{ $country['name'] }}</option>
                                                @endforeach
                                            </select>
                                        </div>
