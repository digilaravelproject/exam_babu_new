...
<head>
    ...
    <title>{{ config('app.name', 'Exam Babu') }}</title>
                ...
                <div class="flex justify-between flex-1 max-w-5xl px-4 mx-auto border-b border-gray-200 md:px-0">
                    ...
                    <div class="flex items-center ml-4 md:ml-6">
                        ...
                        <div class="flex items-center px-3 py-1 text-blue-600 rounded-full bg-blue-50">
                            <span class="mr-1 font-bold">{{ Auth::user()->wallet_balance ?? 0 }}</span> Pts
                        </div>
                        ...
                        <div class="relative ml-3" x-data="{ dropdownOpen: false }">
                            <div>
                                <button @click="dropdownOpen = !dropdownOpen" type="button" class="flex items-center max-w-xs text-sm bg-white rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                                    ...
                                    <span class="hidden ml-2 font-medium text-gray-700 md:block">{{ Auth::user()->first_name }}</span>
                                </button>
                            </div>
                            ...
                            <div x-show="dropdownOpen"
                                ...
                                <div class="px-4 py-2 border-b">
                                    <p class="text-sm text-gray-700">{{ Auth::user()->first_name }} {{ Auth::user()->last_name }}</p>
                                </div>

                                <a href="{{ route('profile.show') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Profile</a>
                                <a href="{{ route('user_subscriptions') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">My Subscriptions</a>
                                <a href="{{ route('user_payments') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">My Payments</a>
